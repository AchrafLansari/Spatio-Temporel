...
PhotoLayerCallback.prototype.getLogo = function(photo) {
  ...
  if (photo.id && photo.id.$t) {
    strRet = "<a href='http://picasaweb.google.com/' target='_provider'><img src='";
    strRet += "https://www.google.com/accounts/lh2/picasaweblogo-en.gif";
    strRet += "' alt='Picasa Web'/></a>";
  } else {
    strRet = "<a href='http://www.flickr.com/' target='_provider'>flickr</a>";
  }
  return strRet;
}
...
function PhotoLayer(map, opt_opts) {
  ...
  me.map = map;
  me.tag = opts.tag || "";
  me.timeFrom = opts.timeFrom || "2008-10-01";
  me.timeTo = opts.timeTo || "2009-02-01";
...
PhotoLayer.prototype.changeTag = function(tag, timeFrom, timeTo) {
  // alert(tag + '.' + timeFrom + '.' + timeTo);
  this.tag = tag;
...
PhotoLayer.prototype.load = function(photoLayer, options) {
  ...
  */
  ...
  var urlTag = (this.tag == "" ? "" : "&tag_mode=all&tags=" + this.tag);
  var url =
  "http://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=35536e08b782e6d772eec5488e965869&extras=geo,owner_name" + urlTag + "&min_taken_date=" + this.timeFrom + "&max_taken_date=" + this.timeTo + "&sort=interestingness-desc&per_page=30&bbox=" + options.west + "," + options.south + "," + options.east + "," + options.north + "&format=json&";   
